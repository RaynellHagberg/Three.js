/***********
 * oscillatingSquares2.js
 * A program to generate waves of squares
 * Raynell Hagberg
 * March 2019
 ***********/
...
function Controls() {
    this.waveRate = 8.0;
    this.create = function () { // started too add GUI controls but ran out of time   
    }
}
...
function initGui() {
    ...
    gui.add(controls, 'waveRate', 1, 20).step(0.1);
...
function heightFunction(delta, dist) {
    return maxHeight * (2 * Math.sinh(0.5 * delta)) * (Math.cosh(-0.5 * delta));
}
